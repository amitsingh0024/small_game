...
const WORLD_SIZE = 50 // Size of the generated world (half-width/height)
...
class Player extends EngineObject {
  ...
  render(): void {
    drawRect(this.pos, this.size, this.color)
    drawText(this.skin, this.pos, 0.5, WHITE)
 ...
 */
function restartGame(currentSkin: string = ':)'): void {
...
function Game() {
      ...
      try {
        await engineInit(
          ...
          () => {
            ...
            // Spawn 2 purple enemies: one near exit, one near block
            // Spawn 2 purple enemies: one near exit, one near block
            // Match generateRandomWorld worldSize
            ...
            if (exitGate) {
              ...
              do {
                ...
                const isOnWall = walls.some(w => w.gridX === exitEnemyX && w.gridY === exitEnemyY)
                const isOnEnemy = enemies.some(e => e.gridX === exitEnemyX && e.gridY === exitEnemyY)
                const isOutOfBounds = Math.abs(exitEnemyX) > WORLD_SIZE || Math.abs(exitEnemyY) > WORLD_SIZE
            ...
            if (blocks.length > 0) {
              ...
              do {
                ...
                const isOnWall = walls.some(w => w.gridX === blockEnemyX && w.gridY === blockEnemyY)
                const isOnEnemy = enemies.some(e => e.gridX === blockEnemyX && e.gridY === blockEnemyY)
                const isOutOfBounds = Math.abs(blockEnemyX) > WORLD_SIZE || Math.abs(blockEnemyY) > WORLD_SIZE
  ...
  return (
    <div className="game-wrapper" style={{ position: 'relative', width: '100%', height: '100%', margin: 0, padding: 0 }}>
      ...
      />
      {showMenu && <MenuScreen onStart={handleStartGame} currentSkin={skin} onSkinSelect={setSkin} />}
